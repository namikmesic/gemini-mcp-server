    ...
    try {
        ...
        if (geminiTool) {
            ...
            if (geminiTool.arguments && Array.isArray(geminiTool.arguments)) {
                const argStrings = geminiTool.arguments.map((a) => `${a.name}${a.required ? ' (required)' : ''}: ${a.description || 'No description'}`).join('\n  - ');
        ...
        logger.error('Error occurred during test:', error.message);
...
runGeminiTest().catch(error => {
    logger.error('Unhandled error in test execution:', error.message);
    process.exit(1);
});
